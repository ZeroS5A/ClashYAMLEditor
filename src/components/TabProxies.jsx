...
import { Server, Link as LinkIcon, Plus, Edit, Trash2, Trash } from 'lucide-react';
import ScrollingText from './ScrollingText';

const TabProxies = ({ proxies, setLinkModalVisible, setEditingProxy, deleteProxy, clearAllProxies }) => (
  <div className="pb-8">
    <div className="sticky top-0 z-20 bg-slate-100/90 dark:bg-slate-950/90 backdrop-blur-md px-4 md:px-8 py-4 mb-6 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button onClick={clearAllProxies} className="px-4 py-2 bg-white border border-red-200 text-red-500 hover:bg-red-50 dark:bg-slate-900 dark:border-red-900/50 dark:text-red-400 dark:hover:bg-red-950/30 rounded-lg flex items-center gap-2 transition-colors"><Trash className="w-4 h-4" /> 清空全部节点</button>
      </div>
    </div>
